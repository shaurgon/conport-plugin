...
- **Workflows** (Claude Code, paid plan)
  - `/conport:task` — deterministic execution of one ConPort task (worktree, independent review, gated merge, close with resolution)
  - `/conport:epic` — the same for a whole epic: the plan is sliced into tasks, they run in waves, and the epic closes on the run's verdict
  - bundled helper `workflows/wf-helper.mjs` — the mechanical half of both (preflight, gate, merge, cleanup, accounting, scouting), so no step of a run depends on a model's mood
